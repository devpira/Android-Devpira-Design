...
/**
 * Created by pira.suriyakumaran on 12/25/2016.
 */

public class PiraToolBar extends Toolbar {
    /**
    ...
    public PiraToolBar(Context context) {
        super(context);
    ...
    public PiraToolBar(Context context, AttributeSet attrs) {
        super(context, attrs);

        this.pWidth = getContext().getResources().getDisplayMetrics().widthPixels;
        this.pHeight = getContext().getResources().getDisplayMetrics().heightPixels;

        typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.PiraToolBar, 0,0);
    }
    ...
    protected void onAttachedToWindow() {
        ...
        if(typedArraySet)
            return;
        try {
            ref = typedArray.getResourceId(R.styleable.PiraToolBar_meParentView,0);
            ...
            viewWidth = (int) typedArray.getFraction(R.styleable.PiraToolBar_meWidth,pWidth,pHeight,0);
            viewHeight = (int) typedArray.getFraction(R.styleable.PiraToolBar_meHeight,pHeight,pWidth,0);
            ...
            setX(typedArray.getFraction(R.styleable.PiraToolBar_meX,pWidth,pHeight,0));
            setY(typedArray.getFraction(R.styleable.PiraToolBar_meY,pHeight,pWidth,0));

            //Following sets margins if any:
            int margin = (int) typedArray.getFraction(R.styleable.PiraToolBar_meMargin, pWidth,pHeight,0);
            if(margin == 0) {                                                                                               //If margin is set then all
                leftMargin = (int) typedArray.getFraction(R.styleable.PiraToolBar_meLeftMargin, pWidth, pHeight, 0);        // individual margins will be overridden
                rightMargin = (int) typedArray.getFraction(R.styleable.PiraToolBar_meRightMargin, pWidth, pHeight, 0);     // to the margin because margin takes priority
                topMargin = (int) typedArray.getFraction(R.styleable.PiraToolBar_meTopMargin, pHeight, pWidth, 0);
                bottomMargin = (int) typedArray.getFraction(R.styleable.PiraToolBar_meBottomMargin, pHeight, pWidth, 0);
            }else{
            ...
            int padding = (int) typedArray.getFraction(R.styleable.PiraToolBar_mePadding,pWidth,pHeight,0);
            if(padding == 0) {                                                                                               //If padding is set then all
                leftPadding = (int) typedArray.getFraction(R.styleable.PiraToolBar_meLeftPadding, pWidth, pHeight, 0);        // individual padding will be overridden
                rightPadding = (int) typedArray.getFraction(R.styleable.PiraToolBar_meRightPadding, pWidth, pHeight, 0);     // to the padding because mePadding takes priority
                topPadding = (int) typedArray.getFraction(R.styleable.PiraToolBar_meTopPadding, pHeight, pWidth, 0);
                bottomPadding = (int) typedArray.getFraction(R.styleable.PiraToolBar_meBottomPadding, pHeight, pWidth, 0);
            }else{
