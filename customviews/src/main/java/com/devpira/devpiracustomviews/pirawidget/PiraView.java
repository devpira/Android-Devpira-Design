...
/**
 * Created by pira.suriyakumaran on 10/17/2016.
 */

public class PiraView extends View {
    ...
    public PiraView(Context context) {
        super(context);
    ...
    public PiraView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onAttachedToWindow() {
        ...
        try {
            ...
            viewWidth = (int) typedArray.getFraction(R.styleable.PiraView_meWidth,pWidth,pHeight,0);
            viewHeight = (int) typedArray.getFraction(R.styleable.PiraView_meHeight,pHeight,pWidth,0);
            ...
            setX(typedArray.getFraction(R.styleable.PiraView_meX,pWidth,pHeight,0));
            setY(typedArray.getFraction(R.styleable.PiraView_meY,pHeight,pWidth,0));

            //Following sets margins if any:
            int margin = (int) typedArray.getFraction(R.styleable.PiraView_meMargin, pWidth,pHeight,0);
            if(margin == 0) {                                                                                               //If margin is set then all
                leftMargin = (int) typedArray.getFraction(R.styleable.PiraView_meLeftMargin, pWidth, pHeight, 0);        // individual margins will be overridden
                rightMargin = (int) typedArray.getFraction(R.styleable.PiraView_meRightMargin, pWidth, pHeight, 0);     // to the margin because margin takes priority
                topMargin = (int) typedArray.getFraction(R.styleable.PiraView_meTopMargin, pHeight, pWidth, 0);
                bottomMargin = (int) typedArray.getFraction(R.styleable.PiraView_meBottomMargin, pHeight, pWidth, 0);
            }else{
            ...
            if(padding == 0) {                                                                                               //If padding is set then all
                leftPadding = (int) typedArray.getFraction(R.styleable.PiraView_meLeftPadding, pWidth, pHeight, 0);        // individual padding will be overridden
                rightPadding = (int) typedArray.getFraction(R.styleable.PiraView_meRightPadding, pWidth, pHeight, 0);     // to the padding because mePadding takes priority
                topPadding = (int) typedArray.getFraction(R.styleable.PiraView_meTopPadding, pHeight, pWidth, 0);
                bottomPadding = (int) typedArray.getFraction(R.styleable.PiraView_meBottomPadding, pHeight, pWidth, 0);
            }else{
